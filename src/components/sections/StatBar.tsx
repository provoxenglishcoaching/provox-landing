...
export default function StatBar() {
  const stats = [
    ...
    { value: 'Groups', label: '2 to 4 people' },
    { value: 'CLT', label: 'Teaching method' },
  ];

  return (
    <section className="border-y" style={{ borderColor: 'var(--border)', background: 'var(--secondary)' }}>
      <div className="max-w-6xl mx-auto px-6 py-10 grid grid-cols-3 gap-6 md:gap-0 md:divide-x" style={{ '--tw-divide-opacity': 1 } as React.CSSProperties}>
        {stats.map((s) => (
